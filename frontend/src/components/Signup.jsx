...
const submitHandler=async(e)=>{
  e.preventDefault();
  try{
    const res = await axios.post("https://gmail-clone-m0qq.onrender.com/api/v1/user/register",input,{
      headers:{
    ...
    <div className="flex items-center justify-center w-screen h-screen ">
     ...
     <form  onSubmit={submitHandler} className="flex flex-col gap-3 bg-white p-4 w-[20%]">
        ...
        <input  onChange={changeHandler} value={input.email} name='email' type='text' placeholder="Email" className=" border border-gray-400 rounded-md px-2 py-1"/>
          ...
          <button type="submit" className="bg-gray-800 p-2 text-white my-2 rounded-md cursor-pointer">Signup</button>
